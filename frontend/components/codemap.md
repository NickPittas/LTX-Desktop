...
- **`ICLoraPanel.tsx`** — exports `ICLoraConditioningType` (`'canny' | 'depth' | null`), `CONDITIONING_TYPES` (None/Canny/Depth), `IC_LORA_ADAPTERS` (flat registry tagged by `AdapterWorkflow`: `standard_video`, `ingredients`, `in_outpainting`, `hdr`, `unavailable`), and the panel. A `useEffect` computes readiness and emits `onChange({videoPath, conditioningType, conditioningStrength, adapterId, maskPath, images, ready, maskGrowPx})`; its controlled in/outpainting-only pipeline selector resets to V1 on adapter or panel reset.
  - **Source-vs-preview invariant**: `videoPath` emitted to backend stays original/primary; `previewPath` is browser-only. MOV/AVI/MKV and unsupported-codec MP4 can be transcoded via `transcodeVideoForPreview` (with progress + race guard); proxy/preview is never emitted as backend `videoPath`.
  - **HDR mode** (`adapterId === 'hdr'`, workflow `hdr`): hides conditioning-type/strength controls, drops the prompt-required expectation (prompt is optional), and renders grounded copy — HDR uses the source video and writes a linear EXR primary with an SDR proxy preview. `GenSpace` preserves the EXR `videoPath` as the primary asset and routes `proxyPath` to the preview player.
  ...
  - **in_outpainting workflow**: requires `maskPath`; exposes the mask-grow control and optional first-frame anchor.
