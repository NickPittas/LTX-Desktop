...
- **Explicit button class maps for the variant surface.** `button.tsx` keeps `BUTTON_BASE_CLASSES`, `BUTTON_VARIANT_CLASSES`, `BUTTON_SIZE_CLASSES`, `ButtonVariant`, and `ButtonSize` module-private; it exports only `Button` and `ButtonProps`. All five primitives hand-assemble their class strings.
...
- **`button.tsx`** — `<Button variant size className ...props>` renders a native `<button>` with the base, variant, size, then consumer classes merged through `cn`. Variants: `default | destructive | outline | secondary | ghost | link`; sizes: `default (h-9) | sm (h-8) | lg (h-10) | icon (h-9 w-9)`. Defaults: `variant='default'`, `size='default'`. Exports `Button` and `ButtonProps`.
...
- **Consumers** — used pervasively across `frontend/components` (e.g. `Button` in `Home`, `LogViewer`, `ModelProfileWizard`, `LtxUpgradePrompt`, `ModelComponentPicker`; `Select`/`Textarea` in `SettingsPanel`; `Tooltip` in `VideoEditor` chrome, `TimelineToolbar`, `VideoEditorLayoutMenu`) and the editor panels under `frontend/views/editor`.
