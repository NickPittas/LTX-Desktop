...
## Design Patterns
- **Schema-first in `project-model.ts`:** every entity is a `z.object(...)` and its TS type is `z.infer<typeof ...>`. Const tuple arrays (`generationModeValues`, `assetTypeValues`, etc.) feed `z.enum(...)` so literals and types stay in lockstep.
