# frontend/types
## Responsibility
TypeScript data-model layer for the renderer. `project-model.ts` is the single source of truth for the project document (Zod schemas + derived types + V1→V2 migration), retaining persisted clip-effect schemas for round-trip compatibility; `project.ts` is a pure static text-preset catalog; `model-profile.ts` re-exports OpenAPI-generated model-profile/adapter types.
...
- **`project.ts` is data-only** (no Zod): `TEXT_PRESETS` array consumed by editor text-overlay UI. Persisted effect types/schemas remain in `project-model.ts` so older projects load and re-save without data loss, although their inactive runtime/UI helpers are gone.
...
- `project.ts` text presets flow into the editor text-overlay UI; persisted project effects round-trip through `project-model.ts`.
...
- **`types/project.ts` ← `types/project-model`:** imports `TextOverlayStyle`.
...
- **`project.ts`:** `TextPreset`, `TEXT_PRESETS`.
