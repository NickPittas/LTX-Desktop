...
- **Reused domain object schemas.** `exportClip`, `exportSubtitle`, `logsResponse`, `backendHealthStatus`, `backendAdminRequest` are extracted as named Zod objects at module scope so they can be referenced from multiple channel definitions and mirrored as TypeScript interfaces in `frontend/types/`. `exportClip.path` is the authoritative primary video; optional `audioPath` is a proxy/playback sidecar used for audio when present.
...
- **Export:** `exportNative` (clips with primary `path` and optional proxy `audioPath`, codec, letterbox, subtitles), zero-argument `exportCancel`.
...
- **Primary-output:** original MOV/project primary remains authoritative for export and model input; MP4 is a sidecar for browser playback and proxy audio only. `exportClip.audioPath` carries that sidecar without replacing primary video. Because schemas drive typing in three places (preload auto-bind, typed-handle, renderer `ElectronAPI`), this is the single edit point to extend the IPC surface.
