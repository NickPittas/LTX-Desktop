...
The **`ipcMain.handle` dispatcher layer**. Wires every channel exposed under `window.electronAPI` (defined by `shared/electron-api-schema.ts`) to a concrete implementation. Video imports preserve their primary and use persistent project-local proxies; guarded managed-file removal independently preflights containment then uses `shell.trashItem` only. Each `register*Handlers()` function is called exactly once from `main.ts` at startup.
...
- **App/system:** `getModelsPath` (ensures `userData/models` exists), `checkGpu` (delegates to `gpu.ts`), `getAppInfo` (`{ version, isPackaged, modelsPath, userDataPath }`), `getResourcePath` (`process.resourcesPath` if packaged, else `null`).
- **First-run/license:** `checkFirstRun` reads `userData/app_state.json`; if absent, falls back to inspecting `model_profiles.json` for an `isActive` profile (returns `{ needsSetup: false, needsLicense: true }`) — otherwise `{ needsSetup: true, needsLicense: true }`. `acceptLicense` / `completeSetup` share `markSetupAndLicenseAccepted`, which merges existing state and writes both completion flags and current ISO timestamps via `writeSettingsFile`. `fetchLicenseText` GETs the LTX-2.3 LICENSE from HuggingFace. `getNoticesText` reads `NOTICES.md` from `app.getAppPath()`.
...
- **Project-asset import (keep-primary + proxy-sidecar):** progress for every path is streamed on `asset:importProgress` (`{jobId, percent, label, done?}`), weighted across phases via an import-job tracker.
  - `addVisualAssetToProject({ srcPath, projectId, type: 'video' | 'image', proxyPath?, jobId? })`:
    ...
    3. **If `proxyPath` is supplied (ProRes/EXR primary):** the primary is preserved verbatim and the proxy MP4 is copied alongside → `projectProxyPath`; thumbnails/dimensions are generated from the proxy copy. Export consumes the primary video directly and uses the sidecar only as `audioPath` when needed; no exporter-created lossy intermediate feeds ProRes.
...
- **`electron/export/`:** `file-handlers.ts` imports `extractVideoFrameToFile`, `findFfmpegPath`, `getVideoDimensions`, `runFfmpeg` from `export/ffmpeg-utils.ts`; `video-processing-handlers.ts` imports `extractVideoFrameToFile`. Export cancellation is signal-owned while import/preview calls retain `isolated: true`, so cancelling an export cannot kill those transcodes. Timeline pass 1 uses the requested delivery codec directly, then the final mux stream-copies video; the existing EXR-directory proxy fallback remains the documented exception.
...
- **ProRes/EXR primary-output (implemented):** the keep-primary + proxy-sidecar design is live — when the renderer passes a `proxyPath`, `addVisualAssetToProject` preserves the primary verbatim and copies the proxy alongside (the `proxyPath` field is part of the `ipcResult` schema), and `transcodeVideoForPreview` mints a non-mutating preview MP4. EXR frames cannot be played by `<video>` at all and ProRes playback is OS-dependent, so a proxy is required for renderer preview of those outputs; preview/transcode `runFfmpeg` calls run with `isolated: true` so the global export-cancel cannot kill an in-flight import/preview transcode.
